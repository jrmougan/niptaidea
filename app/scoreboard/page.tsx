        ...
        <div className="flex gap-2 w-full">
          {DIFFICULTIES.map(({ key, label }) => (
            <Link
              ...
              className={`flex-1 text-center py-2 border text-xs font-mono tracking-widest transition-all ${
                difficulty === key
        ...
        <div className="w-full border border-border-default bg-bg-secondary">
          ...
          {scores.length === 0 ? (
            <div className="py-12 text-center">
              ...
              <p className="text-content-dim/60 font-mono text-xs mt-1">sé el primero en jugar en modo {difficulty}</p>
            </div>
          ) : (
            scores.map((score, i) => (
              <div
                key={score.id}
                className={`grid grid-cols-[2rem_1fr_4rem_4rem_5rem] gap-3 px-4 py-3 border-b border-border-default/50 last:border-0 ${
              ...
              >
                ...
                <span className={`font-mono text-sm uppercase tracking-wider self-center truncate ${i === 0 ? "text-accent-teal text-glow-teal" : "text-content-primary"}`}>
                  {score.name}
                </span>
                <span className="font-mono text-sm text-right self-center">
                  <span className={i === 0 ? "text-accent-orange" : "text-content-muted"}>{score.attempts}</span>
                ...
                <span className="font-mono text-[11px] text-content-dim text-right self-center">
                  {new Date(score.created_at).toLocaleDateString("es-ES", { day: "2-digit", month: "2-digit", year: "2-digit" })}
                </span>
              </div>
            ))
          )}
